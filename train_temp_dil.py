...
def start_train(args, model, train_loader, test_loader, device):
    ...
    threshold_list = {}
    (
        threshold_list["fft64"],
        threshold_list["fft128"],
        threshold_list["fft256"],
        threshold_list["fft512"],
    ) = ([], [], [], [])
    ...
    if args.save_model:
        model_path = (
            ...
            + str(args.lr)
            + "-LRDecStep="
            + str(args.lr_dec_step)
            + "-LRDecRate="
            + str(args.lr_dec_rate)
            + "-Batch="
            + str(args.batch_size)
            + "-epoch="
            + str(args.epochs)
...
if __name__ == "__main__":
    ...
    path = "./data/" + args.task + "/" + str(args.task_num)
    ...
    if args.task == "superclass":
        model = ResNet_PTB(
            ResBlock, SpectralConv1d, args.threshold_ratio, num_classes=5
        ).to(device)
    elif args.task == "subclass":
        model = ResNet_PTB(
            ResBlock, SpectralConv1d, args.threshold_ratio, num_classes=23
        ).to(device)
    elif args.task == "rthym":
        model = ResNet_PTB(
            ResBlock, SpectralConv1d, args.threshold_ratio, num_classes=12
        ).to(device)
    elif args.task == "form":
        model = ResNet_PTB(
            ResBlock, SpectralConv1d, args.threshold_ratio, num_classes=19
        ).to(device)
    else:
