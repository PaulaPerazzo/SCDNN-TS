...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        size = x.size()
        avg, max = self.adapt_avg(x).view(size[0], -1), self.adapt_max(x).view(
            size[0], -1
        )
...
class SpectralConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, init_threshold, signal_shape):
        ...
        # def hook(grad):
        #     print("threshold grad:", grad)

        # self.threshold.register_hook(hook)

        self.scale = 1 / (in_channels * out_channels)
        # signal_shape must be defined various with layers
        self.weights = nn.Parameter(
            self.scale
            * torch.rand(in_channels, out_channels, signal_shape, dtype=torch.float32)
        )
    ...
    def bi_mod_sigmoid(self, threshold_co, fft_co_len, sig_co):
        ...
        threhold_func = torch.sigmoid(0.5 * (sig_co - threshold_co * fft_co_len))
    ...
    def low_sigmoid_step(self, threshold_co, fft_co):
        ...
        sig_co = torch.arange(0, fft_co_len, 1, device=fft_co.device)
    ...
    def forward(self, x):
        ...
        x = torch.fft.irfft(
            self.low_sigmoid_step(threshold, x_ft[:, :, :modes]), n=x.size(-1)
        )
        minus_x = torch.fft.irfft(
            self.high_sigmoid_step(threshold, x_ft[:, :, :modes]), n=x.size(-1)
        )
...
class ResNet_PTB(PytorchModelStructure):
    def __init__(self, config, logger, device):
        super(ResNet_PTB, self).__init__(config, logger, device)
        ...
        self.fft128 = SpectralConv1d(128, 128, init_threshold=0.2, signal_shape=249)
